...
## 6. FPGA Cloud Development Workflow (AWS F2 Example)

The complete workflow for deploying custom RTL to AWS F2:

```
1. Develop RTL locally (Verilog/VHDL)
2. Create AWS FPGA HDK wrapper (axi_interconnect + shell interface)
3. Simulate with Verilator + DPI-C shell model
4. Synthesize on AWS Developer AMI (Vivado WebPack provided free)
5. Run DRC checks via aws ec2 create-fpga-image
6. Create AFI (Amazon FPGA Image)
7. Load AFI onto F2 instance: fpga-load-local-image
8. Test via PCIe DMA from host application
```

### Key Shell Interface Signals (AWS F2)
```verilog
// Your custom logic must interface with these shell signals:
module my_custom_logic (
    // Clock and reset from shell
    input  logic        clk_main_a0,      // Main clock (~250 MHz)
    input  logic        rst_main_n,        // Active-low reset
    
    // AXI-4 interface to shell (DMA path)
    ...
    // PCIS interface (PCIe BAR access from host)
    input  logic [63:0] sh_cl_dma_pcis_awaddr,
    input  logic        sh_cl_dma_pcis_awvalid,
    output logic        sh_cl_dma_pcis_awready,
    // ... (full AXI4-lite signals)
    
    // Interrupt to shell
    output logic [15:0] cl_sh_irq0         // 16 interrupt lines
);
```

---

## 7. Open-Source Cloud FPGA Projects

Several projects aim to reduce vendor lock-in:

| Project | Organization | Goal | Status |
|---|---|---|---|
| **OPAF (Open Programmable Acceleration Framework)** | Intel + community | Open shell framework for Agilex/Stratix 10 | Active — Intel OFS is production-ready |
| **OFS (Open FPGA Services)** | Intel | Open-source FIM (FPGA Interface Manager) for IPU/PAC cards | Active — GitHub Intel OFS |
| **Firesim** | UC Berkeley | FPGA-accelerated cycle-accurate simulation in the cloud | Active — uses AWS F1/F2 |
| **tapasco** | JKU Linz | Toolflow for portable FPGA kernel acceleration | Active — supports Xilinx + Intel |
| **Xilinx Vitis ACCels** | Xilinx/AMD | OpenCL/C++ abstraction layer for Alveo cards | Active — production support |

---

## 8. Security Considerations for Cloud FPGAs

Multi-tenant FPGA environments introduce unique security risks:

| Threat | Description | Mitigation |
|---|---|---|
| **Bitstream reverse engineering** | Extracting RTL netlist from unencrypted bitstream | Enable AES-GCM bitstream encryption; use authenticated loading |
| **Side-channel attacks** | Power analysis to extract keys from neighboring tenant | Physical isolation (dedicated FPGA); no multi-tenancy for crypto |
| **Malicious bitstream** | FPGA design that damages the board (overheating, I/O contention) | Cloud DRC checks before AFI creation |
| **Rowhammer-style attacks** | Aggressive DDR access patterns to cause bit flips in adjacent tenant's memory | Hardware isolation; separate DDR controllers per tenant |
| **Covert channels** | Temperature or power modulation to exfiltrate data | Monitor thermal sensors for anomalous patterns |

---

## 9. Conclusion: The Shift to Managed Services
...
### Decision Flow: Cloud FPGA vs On-Premises
```
Is your workload bursty (not 24/7)?
  ├── Yes → Cloud FaaS may be cost-effective
  │     Is latency < 10 µs required?
  │       ├── Yes → On-premises FPGA (no cloud virtualization overhead)
  │       └── No  → Cloud FaaS is a good fit
  └── No → On-premises FPGA
        Is volume > 100K units/year?
          ├── Yes → Consider ASIC migration
          └── No  → On-premises FPGA is correct
```
...
| Topic | Article |
|---|---|
| Data center FPGA architectures | [Data Center FPGA](datacenter_fpga.md) |
| Market positioning & economics | [FPGA Market Landscape](../00_overview/landscape.md) |
| FPGA prototyping & ASIC emulation | [FPGA Prototyping](../03_design_flow/fpga_prototyping.md) |
| PCIe debugging (cloud uses PCIe) | [PCIe Bringup](../15_case_studies/pcie_bringup.md) |
...
- [Intel Open FPGA Services (OFS)](https://github.com/intel/open-fpga-services)
- [FireSim Documentation](https://fires.im/)
- [TAPASCO Toolflow](https://github.com/esa-tu-darmstadt/tapasco)
